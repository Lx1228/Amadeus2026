...
本项目的语音合成（TTS）需要你**自己克隆音色**，因为音色 ID 绑定个人账号，无法共享。

### 阿里云 CosyVoice（推荐）

1. 注册阿里云账号，开通「百炼」服务：https://bailian.console.aliyun.com/
2. 进入百炼控制台 → 「声音克隆」
3. 上传音色样本（可用本项目的 `public/voice_sample.mp3`，是红莉栖的语音样本）
4. 克隆完成后，会得到一个音色 ID（格式类似 `cosyvoice-v3.5-flash-xxxx-xxxx`）
5. 打开 Amadeus 应用 → 设置 → 语音合成 → 阿里云百炼
6. 填入阿里云 API Key 和**音色 ID**，保存
...
如果不填音色 ID，会尝试使用默认音色（仅作者账号可用，**其他人调用会报错**）。你必须克隆自己的音色才能使用 TTS 功能。
...
A: 音色 ID 绑定个人账号，默认音色仅作者账号可用。你需要在阿里云/MiniMax 控制台自行克隆音色，然后把音色 ID 填入设置。详见上方「语音音色」章节。
